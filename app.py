...
def _risk_level_badge(risk_level: str) -> str:
    return risk_level.upper() if risk_level else "UNKNOWN"
...
st.sidebar.title("Email Forensics")
...
st.subheader(f"Suspicious indicators ({len(indicators)})")
if not indicators:
...
if breakdown:
    ...
    b1.metric(f"Geo/infra (x{weights.get('geo', 0):.2f})", f"{breakdown.get('geo_score', 0):.1f}")
    b2.metric(f"Attachment (x{weights.get('attachment', 0):.2f})", f"{breakdown.get('attachment_score', 0):.1f}")
    b3.metric(f"Campaign (x{weights.get('campaign', 0):.2f})", f"{breakdown.get('campaign_score', 0):.1f}")
    b4.metric(f"Indicator risk (x{weights.get('indicator', 0):.2f})", f"{breakdown.get('indicator_score', 0):.1f}")
    st.caption(
        f"fraud_score = geo x {weights.get('geo', 0):.2f} + attachment x {weights.get('attachment', 0):.2f} "
        f"+ campaign x {weights.get('campaign', 0):.2f} + indicator x {weights.get('indicator', 0):.2f}"
    )
